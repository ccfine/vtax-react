...
export default function checkPermissions(item, options){
    if(item.authorityInfo && item.authorityInfo.length>0){
        //console.log(difference(this.props.options, item.authorityInfo).length > 0)
        return intersection(item.authorityInfo, options).length > 0
    }else{
        return null
    }
}
