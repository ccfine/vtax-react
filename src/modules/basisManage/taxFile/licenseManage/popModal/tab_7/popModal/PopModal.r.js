    ...
    componentWillReceiveProps(props){
        if(props.visible && this.props.visible!==props.visible){
            if(props.id){
                ...
                request.get(`/card/project/build/find/${props.id}`).then(({data}) => {
                    if (data.code === 200) {
    ...
    handleOk(){
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                if(isModify){
                    result = request.put('/card/project/build/update', obj);
                    sucessMsg='修改成功';
                }else if(this.props.action==="add"){
                    obj.projectId = this.props.projectid;
                    result = request.post('/card/project/build/add', obj);
            ...
            <Spin spinning={this.state.formLoading}>
                <Form>
                    <Row>
                        {
                        getFields(form,[{
                            ...setComItem(record.licenseKey,(readonly || this.props.action==="modify"),true,'请输入施工许可证编号'),
                            label:'施工许可证编号',
                    ...
                    <Row>
                        ...
                        getFields(form,[{
                            ...
                            ...setComItem(record.contractPrice,readonly,true,'请输入合同价格（万元）'),
                            label:'合同价格（万元）',
                    ...
                    <Row>
                        ...
                        getFields(form,[{
                            ...
                            componentProps:{
                                ...
                                initialValue:{key:record.stagesId,label:record.stagesItemName}, // 传这里有效
                                selectOptions:{
                                    labelInValue:true,
                                    disabled:readonly,
                                }
                    ...
                    <Row>
                        {
                        getFields(form,[{
                            ...setComItem(record.buildOrg ,readonly,false),
                            label:'施工单位',
                            fieldName:'buildOrg'
                        },{
                            ...setComItem(record.position ,readonly,false),
                            label:'监理单位',
                            fieldName:'supervisionOrg'
                        }])
                    }
                    </Row>
                    <Row>
                        {
                        getFields(form,[{
                            ...setComItem(moment(record.startDate),readonly,true,'请选择合同开工日期'),
                            label:'合同开工日期',
                            fieldName:'startDate',
                            type:'datePicker',
                        },{
                            ...setComItem(moment(record.endDate),readonly,true,'请选择合同竣工日期'),
                            label:'合同竣工日期',
                            fieldName:'endDate',
                            type:'datePicker',
                        }
                ...
                <FileModal id={this.props.id || record.id} visible={this.state.visible} hideModal={this.hideFileModal} url='card/project/build'/>
            </Modal>
        );
    }
}
